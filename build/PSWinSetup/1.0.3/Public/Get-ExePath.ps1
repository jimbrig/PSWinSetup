...
function Get-ExePath {
    ...
    if ($ExpectedInstallLocation) {
        ...
        if ($null -ne $OriginalVsCurrentSystemPathComparison) {
        ...
        if ($null -ne $OriginalVsCurrentEnvPathComparison) {
        ...
        if ($DirectoriesToSearch.Count -gt 0) {
            $DirectoriesToSearchFinal = $($DirectoriesToSearch | Sort-Object | Get-Unique) | ForEach-Object {if (Test-Path $_ -ErrorAction SilentlyContinue) {$_}}
